...
def list_goals(
    ...
    if not current_user.family_id:
        return []
    return db.execute(select(Goal).where(Goal.family_id == current_user.family_id)).scalars().all()
...
def create_goal(
    ...
    if payload.family_id != current_user.family_id:
        raise HTTPException(status_code=403, detail="Cannot create goal for different family")
