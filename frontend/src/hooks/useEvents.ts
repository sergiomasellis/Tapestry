...
export function useEvents(
  ...
  const mergedEvents = useMemo(() => {
    ...
    return eventList.map((ev) => {
      if (ev.id === dragState.id) {
        ...
        const finalStart = addDays(newStart, dragState.dayOffset);
        const finalEnd = addDays(newEnd, dragState.dayOffset);
  ...
  const commitDrag = useCallback(
    async (drag: NonNullable<DragState>) => {
      ...
      newStart.setHours(h, m, 0, 0);
      newEnd.setTime(newStart.getTime() + drag.durationMinutes * 60000);
